...
describe('official OpenAPI adapter', () => {
  ...
  it('keeps inactive arrangement content readable without treating it as live', () => { const state = enrichPlaylistContext(normalizeCanonicalState({ revision: 1, position, activePlaylist: active, status }), { id: id('playlist-a'), items: [item('item-a', 'Full')] } as never); const inactive = playlistItemContext({ id: 'playlist-a', name: 'Playlist A' }, normalizePlaylistItems({ id: id('playlist-a'), items: [item('item-b', 'Chorus Only')] } as never)[0])!; expect(inactive.cacheKey).toContain('Chorus Only'); expect(isCurrentContext(state, inactive)).toBe(false); expect(canTriggerPresentationCue(state, inactive)).toBe(false); });
  ...
  it('keeps a playlist presentation visible while its detail is still loading', () => { const state = normalizeCanonicalState({ revision: 1, position, activePlaylist: active, status }); expect(activePlaylistPresentationContext(state)).toMatchObject({ source: 'playlist', playlistId: 'playlist-a', playlistItemId: 'item-a', presentationId: 'presentation-a' }); });
