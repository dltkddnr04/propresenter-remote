...
describe('official OpenAPI adapter', () => {
  ...
  it('keeps inactive playlist cues actionable without treating them as live', () => { const state = enrichPlaylistContext(normalizeCanonicalState({ revision: 1, position, activePlaylist: active, status }), { id: id('playlist-a'), items: [item('item-a', 'Full')] } as never); const inactive = playlistItemContext({ id: 'playlist-a', name: 'Playlist A' }, normalizePlaylistItems({ id: id('playlist-a'), items: [item('item-b', 'Chorus Only')] } as never)[0])!; expect(inactive.cacheKey).toContain('Chorus Only'); expect(isCurrentContext(state, inactive)).toBe(false); expect(isActivePlaylistContext(state, inactive)).toBe(false); });
