...
function TopNav({ settings, active, title, following, onFollow, onSettings, onConnection }: { settings: Settings; active?: ActiveState; title: string; following: boolean; onFollow: () => void; onSettings: () => void; onConnection: () => void }) { return <nav className="top-nav"><strong>ProPresenter Remote</strong><span className="top-playlist">{title}</span><div className="top-actions"><button className="top-live-badge" onClick={onConnection} aria-label="연결 정보 수정" title="연결 정보 수정"><span className="status-dot" />{settings.host}:{settings.port} · {active ? '연결됨' : '연결 확인 중'}</button><button className="top-follow-button" disabled={following} onClick={onFollow}>{following ? '현재 슬라이드 추적 중' : '현재 슬라이드 따라가기'}</button><button onClick={() => window.location.assign('/remote')}>리모컨</button><button onClick={onSettings}>앱 설정</button></div></nav>; }
...
function SidebarBrowser({ base, source, active, playlists, selectedPlaylist, playlistItems, playlistLoading, playlistError, onSourceChange, onPlaylistSelect, onLibraryPresentation, onPresentationSelect }: { base: string; source: 'playlist' | 'library'; active?: ActiveState; playlists: Playlist[]; selectedPlaylist: Playlist | undefined; playlistItems: ApiObject[]; playlistLoading: boolean; playlistError: Error | null; onSourceChange: (source: 'playlist' | 'library') => void; onPlaylistSelect: (playlistId: string) => void; onLibraryPresentation: (libraryId: string, presentation: ApiObject) => void; onPresentationSelect: (presentationId: string) => void }) {
  ...
  const selectPlaylist = (playlistId: string) => { onPlaylistSelect(playlistId); onSourceChange('playlist'); };
  const selectedName = source === 'library' ? selectedLibrary?.name : selectedPlaylist?.name;
  const selectedItems = source === 'library' ? libraryItemsQuery.data || [] : playlistItems;
  ...
  return <div className="sidebar-browser">
    <div className="sidebar-source-tabs" role="tablist" aria-label="콘텐츠 종류">
      <button className={`sidebar-source-tab ${source === 'library' ? 'active' : ''}`} role="tab" aria-selected={source === 'library'} onClick={() => onSourceChange('library')}>라이브러리</button>
      <button className={`sidebar-source-tab ${source === 'playlist' ? 'active' : ''}`} role="tab" aria-selected={source === 'playlist'} onClick={() => onSourceChange('playlist')}>재생목록</button>
    </div>
    ...
    <section className="sidebar-selected-items" aria-label={selectedName ? `${selectedName} 항목` : '선택한 항목'}>
      <div className="sidebar-selected-heading">{selectedName || '항목을 선택하세요'}</div>
      {selectedLoading && <p className="sidebar-status">항목 불러오는 중…</p>}
